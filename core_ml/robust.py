...
for noise in noise_levels:
    print(f"\nTraining with asymmetric noise ratio: {noise}")
    ...
    noisy_train_dataset = add_label_noise(noisy_train_dataset, asymmetric_noise_ratio=noise)
    ...
    train_model(model_ce, train_loader, criterion_ce, optimizer_ce, epochs=num_epochs)
    acc_ce = test_model(model_ce, test_loader)
    results['CrossEntropy'].append(acc_ce)
    print(f"CrossEntropy Accuracy: {acc_ce:.4f}")
    ...
    train_model(model_focal, train_loader, criterion_focal, optimizer_focal, epochs=num_epochs)
    acc_focal = test_model(model_focal, test_loader)
    results['FocalLoss'].append(acc_focal)
    print(f"FocalLoss Accuracy: {acc_focal:.4f}")
    ...
    train_model(model_nce, train_loader, criterion_nce, optimizer_nce, epochs=num_epochs)
    acc_nce = test_model(model_nce, test_loader)
    results['Normalized CrossEntropy'].append(acc_nce)
    print(f"Normalized CrossEntropy Accuracy: {acc_nce:.4f}")
    ...
    train_model(model_nfl, train_loader, criterion_nfl, optimizer_nfl, epochs=num_epochs)
    acc_nfl = test_model(model_nfl, test_loader)
    results['Normalized FocalLoss'].append(acc_nfl)
    print(f"Normalized FocalLoss Accuracy: {acc_nfl:.4f}")
    ...
    train_model(model_apl, train_loader, criterion_apl, optimizer_apl, epochs=num_epochs)
    acc_apl = test_model(model_apl, test_loader)
    results['APL'].append(acc_apl)
    print(f"APL Accuracy: {acc_apl:.4f}")
